...
def find_similar(title, ratings_df, movies_df, pivoted_df, n, shared_thresh=3, total_thresh=10, more_data=False):
    ...
    """
    if find_by_name(title, movies_df) == None:
        print('ERROR: Movie name not found')
    else:
        # get movie id using name
        movie_id = int(movies_df.loc[movies_df['title'] == find_by_name(title, movies_df), 'movieId'])
